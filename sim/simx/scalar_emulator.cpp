...
ScalarEmulator::ipdom_entry_t::ipdom_entry_t(const ThreadMask &tmask, Word PC)
  : tmask(tmask)
  , PC(PC)
  , fallthrough(false)
{}

ScalarEmulator::ipdom_entry_t::ipdom_entry_t(const ThreadMask &tmask)
  : tmask(tmask)
  , fallthrough(true)
{}

ScalarEmulator::warp_t::warp_t(Arch& arch)
  : ireg_file(arch.num_threads(), std::vector<Word>(MAX_NUM_REGS))
  , freg_file(arch.num_threads(), std::vector<uint64_t>(MAX_NUM_REGS))
  , uuid(0)
{}

void ScalarEmulator::warp_t::clear(uint64_t startup_addr) {
...
ScalarEmulator::ScalarEmulator(Arch &arch, const DCRS &dcrs, ScalarCore* core)
    : arch_(arch)
    , dcrs_(dcrs)
    , scalarcore_(core)
...
ScalarEmulator::~ScalarEmulator() {
  this->cout_flush();
}

void ScalarEmulator::clear() {
...
void ScalarEmulator::attach_ram(RAM* ram) {
...
instr_trace_t* ScalarEmulator::step() {
  ...
  uint32_t g_wid = scalarcore_->id() * arch_.num_warps() + scheduled_warp;
  uint64_t uuid = (uint64_t(g_wid) << 32) | instr_uuid;
#else
  uint64_t uuid = 0;
#endif

  DPH(1, "Fetch: cid=" << scalarcore_->id() << ", wid=" << scheduled_warp << ", tmask=");
...
bool ScalarEmulator::running() const {
  return active_warps_.any();
}

int ScalarEmulator::get_exitcode() const {
  return warps_.at(0).ireg_file.at(0).at(3);
}

void ScalarEmulator::suspend(uint32_t wid) {
  assert(!stalled_warps_.test(wid));
  stalled_warps_.set(wid);
}

void ScalarEmulator::resume(uint32_t wid) {
  if (wid != 0xffffffff) {
    assert(stalled_warps_.test(wid));
...
bool ScalarEmulator::wspawn(uint32_t num_warps, Word nextPC) {
...
bool ScalarEmulator::barrier(uint32_t bar_id, uint32_t count, uint32_t wid) {
  if (count < 2)
    return true;
  ...
  barrier.set(wid);
  DP(3, "*** Suspend core #" << scalarcore_->id() << ", warp #" << wid << " at barrier #" << bar_idx);

  if (is_global) {
    // global barrier handling
    if (barrier.count() == active_warps_.count()) {
      scalarcore_->socket()->barrier(bar_idx, count, scalarcore_->id());
      barrier.reset();
    }
  } else {
    ...
    if (barrier.count() == (size_t)count) {
      ...
      for (uint32_t i = 0; i < arch_.num_warps(); ++i) {
        if (barrier.test(i)) {
          DP(3, "*** Resume core #" << scalarcore_->id() << ", warp #" << i << " at barrier #" << bar_idx);
...
#else
void ScalarEmulator::icache_read(void *data, uint64_t addr, uint32_t size) {
    mmu_.read(data, addr, size, 0);
}
#endif

#ifdef VM_ENABLE
void ScalarEmulator::set_satp(uint64_t satp) {
...
#ifdef VM_ENABLE
void ScalarEmulator::dcache_read(void *data, uint64_t addr, uint32_t size) {
...
void ScalarEmulator::dcache_read(void *data, uint64_t addr, uint32_t size) {
  auto type = get_addr_type(addr);
  if (type == AddrType::Shared) {
    scalarcore_->local_mem()->read(data, addr, size);
  } else {
    mmu_.read(data, addr, size, 0);
  }
...
#ifdef VM_ENABLE
void ScalarEmulator::dcache_write(const void* data, uint64_t addr, uint32_t size) {
...
void ScalarEmulator::dcache_write(const void* data, uint64_t addr, uint32_t size) {
  ...
  if (addr >= uint64_t(IO_COUT_ADDR)
   && addr < (uint64_t(IO_COUT_ADDR) + IO_COUT_SIZE)) {
    ...
    if (type == AddrType::Shared) {
      scalarcore_->local_mem()->write(data, addr, size);
    } else {
      mmu_.write(data, addr, size, 0);
    }
  }
...
void ScalarEmulator::dcache_amo_reserve(uint64_t addr) {
  auto type = get_addr_type(addr);
...
bool ScalarEmulator::dcache_amo_check(uint64_t addr) {
  auto type = get_addr_type(addr);
  if (type == AddrType::Global) {
...
void ScalarEmulator::writeToStdOut(const void* data, uint64_t addr, uint32_t size) {
  if (size != 1)
    std::abort();
...
void ScalarEmulator::cout_flush() {
...
Word ScalarEmulator::get_csr(uint32_t addr, uint32_t tid, uint32_t wid) {
  auto core_perf = scalarcore_->perf_stats();
  switch (addr) {
  case VX_CSR_SATP:
#ifdef VM_ENABLE
  ...
  case VX_CSR_MHARTID:    return (scalarcore_->id() * arch_.num_warps() + wid) * arch_.num_threads() + tid;
  case VX_CSR_THREAD_ID:  return tid;
  case VX_CSR_WARP_ID:    return wid;
  case VX_CSR_CORE_ID:    return scalarcore_->id();
    ...
    if ((addr >= VX_CSR_MPM_BASE && addr < (VX_CSR_MPM_BASE + 32))
     || (addr >= VX_CSR_MPM_BASE_H && addr < (VX_CSR_MPM_BASE_H + 32))) {
      ...
      case VX_DCR_MPM_CLASS_MEM: {
        auto proc_perf = scalarcore_->socket()->cluster()->processor()->perf_stats();
        auto cluster_perf = scalarcore_->socket()->cluster()->perf_stats();
        auto socket_perf = scalarcore_->socket()->perf_stats();
        auto lmem_perf = scalarcore_->local_mem()->perf_stats();
        switch (addr) {
...
void ScalarEmulator::set_csr(uint32_t addr, Word value, uint32_t tid, uint32_t wid) {
  __unused (tid);
  switch (addr) {
  case VX_CSR_FFLAGS:
...
uint32_t ScalarEmulator::get_fpu_rm(uint32_t func3, uint32_t tid, uint32_t wid) {
  return (func3 == 0x7) ? this->get_csr(VX_CSR_FRM, tid, wid) : func3;
}

void ScalarEmulator::update_fcrs(uint32_t fflags, uint32_t tid, uint32_t wid) {
  if (fflags) {
